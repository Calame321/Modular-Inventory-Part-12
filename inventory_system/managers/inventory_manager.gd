...
func set_hand_position( pos ):
	...
	if item_in_hand_node:
		item_in_hand_node.rect_position = ( pos - item_offset )
...
func add_inventory( inv ) -> void:
	inv.connect( "content_changed", self, "_on_inventory_content_changed" )
...
func _on_stack_splitted( slot_node, new_quantity ):
	...
	set_hand_position( slot_node.rect_global_position )
...
func _on_gui_input_slot( event : InputEvent, slot_node : Inventory_Slot_Node ):
	...
	if slot.item and slot.item.quantity > 1 and item_in_hand_node.item == null and event is InputEventMouseButton and event.pressed and event.button_index == BUTTON_RIGHT and Input.is_key_pressed( KEY_SHIFT ):
		split( slot_node )
		...
		if event.button_index == BUTTON_LEFT:
			...
			if item_in_hand_node.item and had_empty_hand:
				item_offset = event.global_position - slot_node.rect_global_position
